    ...
    <div class="container-fluid">
        <div class="archive-header mb-3">
            <div class="row align-items-center">
                <div class="col-xl-3">
                    <h1 class="mb-4">{{ $categories_detail->category_name }}</h1>
                    <div class="breadcrumb">
                        <a href="{{ url(' ') }}" rel="nofollow"><i class="fa fa-home mx-1"></i>Home</a>
                        <span><i class="fa fa-angle-right mr-5"></i></span> {{ $categories_detail->category_name }}
                    </div>
                </div>
                <div class="col-xl-9 text-end d-none d-xl-block">
                    <ul class="tags-list">
                        @foreach(DB::table('sub_categories')->where('category_name' , $categories_detail->id)->limit(5)->get() as $r)
                        <li class="hover-up">
                            <a href="{{ url('category') }}/{{ $categories_detail->url }}/{{ $r->url }}">{{$r->subcat_name}}</a>
                        </li>
                        @endforeach
                    </ul>
                </div>
